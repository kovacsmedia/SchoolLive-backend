...
router.post("/register", async (req, res) => {
    try {
        ...
        const pending = await client_1.prisma.pendingDevice.upsert({
...
router.get("/status/:pendingId", async (req, res) => {
    try {
        ...
        if (pending) {
            await client_1.prisma.pendingDevice.update({
                where: { id: pendingId },
        ...
        const session = await client_1.prisma.deviceProvisionSession.findFirst({
 ...
 */
router.get("/pending", authJwt_1.authJwt, async (req, res) => {
    try {
        const user = req.user;
 ...
 */
router.post("/activate", authJwt_1.authJwt, async (req, res) => {
    try {
        const user = req.user;
        if (user?.role !== "SUPER_ADMIN" && user?.role !== "TENANT_ADMIN") {
            return res.status(403).json({ error: "Forbidden" });
        }
        const { pendingId, tenantId, name, deviceClass, wifiSsid, wifiPassword, orgUnitId } = req.body ?? {};
...
router.post("/provision/start", authJwt_1.authJwt, async (req, res) => {
    try {
        ...
        const device = await client_1.prisma.device.findFirst({ where: { serialNumber } });
...
router.post("/provision/confirm", async (req, res) => {
    try {
        ...
        await client_1.prisma.deviceProvisionSession.delete({ where: { id: matchedSession.id } });
        return res.json({
