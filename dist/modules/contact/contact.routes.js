...
function sendSmtp(opts) {
    return new Promise((resolve, reject) => {
        ...
        const lines = [];
        let step = 0;
        const sock = opts.secure
            ? tls.connect(opts.port, opts.host, { servername: opts.host })
        ...
        sock.on("data", (chunk) => {
            ...
            if (isNaN(code))
                return;
            if (step === 0 && code === 220) {
                sock.write(`EHLO schoollive.hu${crlf}`);
                step++;
            }
            else if (step === 1 && (code === 250 || code === 220)) {
                // may get multiline 250; wait for last line (no dash after code)
                if (line[3] === "-")
                    return; // still multiline
                sock.write(`AUTH LOGIN${crlf}`);
                step++;
            }
            else if (step === 2 && code === 334) {
                sock.write(b64(opts.user) + crlf);
                step++;
            }
            else if (step === 3 && code === 334) {
                sock.write(b64(opts.pass) + crlf);
                step++;
            }
            else if (step === 4 && code === 235) {
                sock.write(`MAIL FROM:<${opts.from}>${crlf}`);
                step++;
            }
            else if (step === 5 && code === 250) {
                sock.write(`RCPT TO:<${opts.to}>${crlf}`);
                step++;
            }
