...
exports.startBellsScheduler = startBellsScheduler;
...
async function tick() {
    ...
    if (sec > DISPATCH_WINDOW_S)
        return;
    try {
        const tenants = await client_1.prisma.tenant.findMany({ select: { id: true } });
...
async function processTenant(tenantId, todayMidnight, hour, minute, now) {
    ...
    if (vpDevices.length === 0)
        return;
    const calDay = await client_1.prisma.bellCalendarDay.findUnique({
    ...
    if (calDay?.template?.bells?.length) {
        ...
        const def = await client_1.prisma.bellScheduleTemplate.findFirst({
    ...
    for (const bell of dueBells) {
        ...
        const onlineIds = allVpIds.filter(id => SyncEngine_1.SyncEngine.isDeviceOnline(id));
        const offlineIds = allVpIds.filter(id => !SyncEngine_1.SyncEngine.isDeviceOnline(id));
        if (onlineIds.length > 0) {
            ...
            console.log(`[BELLS-SCHEDULER] 🔔 SyncCast: ${bell.hour}:${String(bell.minute).padStart(2, "0")}` +
        ...
        if (offlineIds.length > 0) {
            ...
            console.log(`[BELLS-SCHEDULER] 🔔 DB queue: ${bell.hour}:${String(bell.minute).padStart(2, "0")}` +
                ` → ${offlineIds.length} offline VP | tenant: ${tenantId}`);
        }
    }
}
function startBellsScheduler() {
    if (_running)
        return;
    _running = true;
