...
router.post("/ytplaylists/build-custom", authJwt, requireTenant, async (req: Request, res: Response) => {
  ...
  const { name, items } = req.body as {
    name: string;
    items: Array<{ url: string; title: string; source: "youtube" | "gdrive" | "upload" }>;
  };
  ...
  (async () => {
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        const item    = items[i];
        const outBase = path.join(tmpDir, `track_${String(i).padStart(3, "0")}`);
        ...
        if (item.source === "youtube") {
          ...
          if (!found) throw new Error(`yt-dlp letöltés sikertelen: ${item.url}`);
          ...
          const found = fs.readdirSync(tmpDir).find(f => f.startsWith(`track_${String(i).padStart(3,"0")}`));
          if (!found) throw new Error(`Drive letöltés sikertelen: ${item.url}`);
          downloadedFiles.push(path.join(tmpDir, found));
